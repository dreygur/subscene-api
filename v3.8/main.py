...
class SubScene(wx.Frame):
    ...
    def __init__(self, parent, id):
        ...
        self.pList=['bluray','hc','hdrip','dvdrip','camrip','webdl','web-dl', 'webrip', 'brrip']
        self.langList=['English', 'Bengali', 'Arabic', 'Indonesian', 'Hindi']
        #self.lanGuage='English'
        self.install()

        try:
            with open(r'C:\Windows\Temp\infoFileSubLang', 'r') as j:
                #print(load(j))
                self.lanGuage=load(j)['defaultLanguage']
        except Exception as e:
            print('Line 102: ' + str(e))
            self.chooseDefaultLang()
        ...
        if len(sys.argv)==1:
            ...
            b3=wx.Button(self.bitmap1, -1, 'Select Default Language', pos=(205, 370), size=(180, 30))
            ...
            self.Bind(wx.EVT_BUTTON, self.chooseDefaultLang, b3)
    ...
    def chooseDefaultLang(self, e=None):
        m=wx.SingleChoiceDialog(None, 'Please select default Language for subtitle: ', 'Language Choice', self.langList)
        if m.ShowModal()==wx.ID_OK:
            st=m.GetStringSelection()
            self.lanGuage=st
            with open(r'C:\Windows\Temp\infoFileSubLang', 'w') as j:
                dump({'defaultLanguage': st}, j)
